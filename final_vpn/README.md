...
这是一个教学和实验性质的 VPN 项目，实现了完整的加密通信通道，支持多客户端同时连接，**并可作为互联网网关使用**。项目采用 Workspace 架构，代码清晰易懂，适合学习网络编程和密码学应用。
...
- ✅ **🌐 网关模式**：服务端可作为互联网网关，实现 NAT 转发（**新功能**）
- ✅ **🚀 全隧道模式**：客户端可将所有流量路由到VPN（**新功能**）
...
```
┌─────────────┐         ┌──────────────┐         ┌──────────────┐
│  Client A   │◄--VPN--►│ VPN Server   │◄--NAT--►│   Internet   │
│ (10.0.0.2)  │         │ (10.0.0.1)   │         │  (公网服务)   │
└─────────────┘         │   TUN设备    │         └──────────────┘
                        │  IP转发      │
                        │  iptables    │
                        └──────────────┘
```
**用途**：让客户端 A 通过服务器的网络访问互联网（隐藏真实 IP、突破网络限制）
...
```bash
vpn_client <虚拟IP> [服务器地址] [--full-tunnel]

# 示例：
vpn_client 10.0.0.2                          # 连接本地服务器（点对点）
vpn_client 10.0.0.2 192.168.1.100:9000       # 指定局域网服务器
vpn_client 10.0.0.2 example.com:9000         # 连接远程服务器（支持域名）
vpn_client 10.0.0.2 1.2.3.4:9000 --full-tunnel  # 全隧道模式（所有流量走VPN）
```
...
- `--full-tunnel`：全隧道模式，将所有流量（包括互联网）路由到VPN（需要服务端开启 `--gateway`）
